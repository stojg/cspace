...
func (b *BloomEffect) Render(inTexture uint32) uint32 {
	...
	for i := 0; i < blurAmount; i++ {
		shaderBlur.Use()
		gl.BindFramebuffer(gl.DRAW_FRAMEBUFFER, b.pingBuffers[horizontal].fbo)
		gl.ActiveTexture(gl.TEXTURE0)
		...
		if firstIteration {
			...
			gl.BindTexture(gl.TEXTURE_2D, b.pingBuffers[horizontal].texture)
		}
		renderQuad()
	}
	...
	gl.BindTexture(gl.TEXTURE_2D, b.pingBuffers[1].texture)
	renderQuad()
